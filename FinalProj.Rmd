...
# State name, change from abbrv to normal name
...
model2 <- lm(data = data, 
         eating ~ state + age + sex + race + famincome)
summary(model2)
...
# I want to create a graph that lets people pick states and show the average amount of time spent on each activity. x = activity, y = hours. How would i calculate the average time spent for each state?
# I want to make a similar one looking at income distribution as well
# Unsure of how to filter the data

```

```{r}
model4 <- lm(data = data, 
         working ~ sex + famincome + edu)
summary(model4)
```
